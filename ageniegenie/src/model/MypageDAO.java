...
public class MypageDAO {
   ...
   final String sql_random = "SELECT A.* FROM (SELECT RANK() OVER (ORDER BY VIEWNUM DESC), TITLE, ARTIST FROM (SELECT * FROM GENIE WHERE GNUM NOT IN(SELECT GNUM FROM MYPAGE))  WHERE (ROWNUM BETWEEN 1 AND 5)  ORDER BY DBMS_RANDOM.RANDOM)A WHERE ROWNUM<=1";
